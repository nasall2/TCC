...
Timing is crucial to the functioning of a television broadcasting system, because it is necessary to guarantee the reproduction of video and audio simultaneously. To enable such a timing, a complex system is defined in the standard. The key concept is to define a System-wide Time Clock, sample this clock and transmit it encoded in the TS packets. The field that carries the samples is the PCR, detailed below. In the decoder side, the PCR feeds a PLL module that recreates the original STC which then is used in the audio and video streams sync process. Elementary streams, in their turn, carry temporal references to the STC in PTS and DTS fields, explained below, and so are played based on the same time reference as the encoder.
