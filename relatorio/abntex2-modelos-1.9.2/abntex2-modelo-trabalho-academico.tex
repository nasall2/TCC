...
Video and audio information are transported in separate streams that need to be reproduced simultaneously on the receiver side. This is why timing is crucial to the operation of a television broadcasting system. 

The key concept introduced in the ISO standard is the System Time Clock (STC) module, which is a reference time base for all the streams within a program. The STC is encoded in the two PCR fields, one in terms of units of 90KHz and the other in parts of 27MHz, which are incremented by a clock signal derived from a 27MHz reference oscillator. This reference may be a GPS satellite sync method or an atomic clock oscillator \cite{}.

In the decoder, a digital PLL (Phase Locked Loop) mechanism synchronizes the local time reference to one in the encoder side. At the moment that each PCR arrives at the decoder, the value is compared with the current local STC value 

Elementary streams, in their turn, carry temporal references to the STC and are played based on the same time reference as the encoder. The field that carries the STC samples is the PCR, detailed below. Elementary Streams carry timing information in the PTS and DTS fields.
