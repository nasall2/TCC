...
The differences between the original ISDB and the modified standard used in Brazil mainly relate to video encoding and interactivity platform. In order to promote the development of national industry , the government decided to adopt an open source interactive platform, developed mainly with Ginga, a technology developed mainly by PUC-RJ\cite{PUCRJ}. Through this tool the broadcaster can, for example, deliver useful information to the low-income population without access to the \textit{Internet}, as an application sugested by one Brazilian Bank in 2010 \cite{caixa}. Another application is a project recently developed by the brazilian public comunication company (EBC), called "Brasil 4D" \cite{consultas}. The users can make appointments with doctors or schedule meetings to solve social security issues, or yet check job offers in real time, all this through the TV remote control. However , the similarity of the Brazilian system with the international standard and the fact that interactive interfaces will not be mandatory until 2015 led to the lack of interest of many broadcasters in the development of applications, so that currently very little is invested in the creation of interactive digital television in Brazilian territory.
 ...
 A digital television system is composed basically by the group of equipments presented in Picture \ref{fig:diagrama_blocos_tvd}. At the beggining of the dignal flow, there are the video and audio capturing elements, such as cameras and microphones, and the signal may be analog or digital depending on the capture technology used. Once captured, the video and audio are passed through the H264 and AAC encoders, respectively, which have input interfaces compatible to the capture equipments. The output of the H264 or AAC encoders are standardized bitstreams known as Elementary Streams. Once the elementary streams leave the encoders, they enter the muxer, where all the streams are packetized and embedded into one single stream, the Transport Stream. What follows is one key process for the system robustness: the Transport Stream receives a layer of Error Correction Code to resist to the noisy, multipath channel between the broadcaster and the receiver, becoming the Broadcast Transport Stream (BTS). Finally, a digital modulation is applied to the BTS and than it is sent to the antenna.
...
\end{figure}
 ...
 The main difference between analog and digital TV broadcasting systems, at the modulation level, is that in analog TV the multiplexing is frequential, i.e., the video signal is sent in a frequency interval within the channel bandwidth and the audio in another frequency interval. In digital technology, this multiplexing happens in time domain: the muxer output has constant bitrate, and for fractions of second only one packet from one of the Packetized Elementary Streams is sent to the transmitter. In the other side of the channel, the receiver is conceived in such a way that after passing the error correction layer, a \textit{demuxer} takes the Transport Stream and splits it back into separate Elementary Streams. They are then sent to the decoders and to audiovisual output devices. One challenge succesfully accomplished in the time domain multiplexing scheme used here is to synchronize the reproduction of all the streams of one channel. It is certanly disturbing to the viewer to watch delayed video or audio outputs, so presentation time stamps were added periodically into the streams so they can be reproduced in sync.
...
% ---
In this section is described the MPEG2 Transport Stream along with its PSI tables. The Picture \ref{fig:TS_iso13818}, copied from the standard as is, presents a schema of the construction of a TS packet. From the picture, it can be seen that a packet is roughly formed with 188 bytes of data. The mandatory header has 4 bytes, and goes from the \textit{sync byte} to the \textit{continuity counter}. The packet data might also contain the  \textit{Adaptation Field}, a sort of header extension, with aditional data that helps the decoder to decode and preset data more optimally. The elementary streams, or \textit{ES}, are split into packets of variable length and together constitute a PES ('P' for packetized). Apart from the 4 mandatory bytes for the header, the other 184 bytes might be filled in three differnt ways:

\begin{itemize}

\item{ with the \textit{Adaptation Field} only;}

\item{ with both the Adaptation Field and PES data;}

\item{or finally with sections of data tables.}
